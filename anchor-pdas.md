...
- Обмеження `init_if_needed` використовується для умовної ініціалізації нового акаунту.
- Обмеження `realloc` використовується для перерозподілу простору на існуючому акаунті.
- Обмеження `close` використовується для закриття акаунту та повернення оплати за оренду.
