...
```rust
    ...
    ```
2. Якщо для програми, інструкцію(ї) якої потрібно викликати, немає допоміжного модуля, ви можете скористатися `invoke` та `invoke_signed`. Насправді, вихідний код функції допомоги `mint_to`, на яку було посилання вище, показує приклад використання `invoke_signed`, коли передається `CpiContext`. Ви можете слідувати подібному шаблону, якщо вирішите використовувати структуру акаунтів та `CpiContext` для організації та підготовки вашого CPI.
