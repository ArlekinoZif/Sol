---
назва: Arbitrary CPI (Довільний міжпрограмний виклик)
завдання:
- Пояснити ризики безпеки, пов'язані з запитом CPI до невідомої програми
- Показати, як модуль CPI Anchor запобігає цьому (запиту до невідомої програми) при здійсненні CPI від однієї програми Anchor до іншої
- Безпечно та надійно здійснювати CPI від програми Anchor до довільної не-Anchor програми
---

# Стислий виклад

- Щоб згенерувати CPI, цільову програму потрібно передати у викликаючу інструкцію як акаунт. Це означає, що в інструкцію може бути передана будь-яка цільова програма. Ваша програма повинна перевіряти некоректні або неочікувані програми.
...
Відсутність перевірок програм створює можливість зловмисникам передати іншу програму, ніж очікувалося, змушуючи оригінальну програму викликати інструкцію цієї невідомої програми. Немає можливості передбачити наслідки цього CPI. Вони залежать від логіки програми (як оригінальної, так і невідомої програми), а також від інших акаунтів, які вже вписані в оригінальну інструкцію.
...
Залежно від програми, яку ви викликаєте вашою CPI, ви можете або жорстко закодувати (дані зашиваються жорстко в програму і не можуть бути змінені без правки коду програми) адресу ID очікуваної програми, або використовувати бібліотеку Rust програми для отримання адреси програми, якщо це можливо. У прикладі вище крейт `spl_token` надає адресу SPL Token Program.
...
Наведена нижче програма показує приклад використання модуля CPI для SPL Token Program, щоб виконати переказ, продемонстрований в попередніх прикладах.
...
В залежності від програми, до якої ви робите CPI, ви можете використовувати тип акаунту Anchor [`Program` account type](https://docs.rs/anchor-lang/latest/anchor_lang/accounts/program/struct.Program.html), щоб підтвердити програму в структурі перевірки акаунту. Між бібліотеками [`anchor_lang`](https://docs.rs/anchor-lang/latest/anchor_lang) та [`anchor_spl`](https://docs.rs/anchor_spl/latest/) надаються за замовчуванням такі типи `Program`:
...
Для цього вже існує тест у директорії `tests`. Він довгий, але присвятіть хвилину, щоб ознайомитися з ним, перш ніж ми розглянемо його разом:
...
Цей тест відіграє сценарій, де звичайний і атакуючий гравці створюють своїх персонажів. Тільки атакуючий гравець передає ідентифікатор програми фальшивих метаданих замість справжньої програми з метаданими. І оскільки інструкція `create_character_insecure` не має перевірок програми, вона все ще виконується.
...
Якщо ви хочете випробувати свої навички, спробуйте зробити це самостійно, перш ніж рухатися далі.

Ми розпочнемо з оновлення нашого оператора `use` у верхній частині файлу `lib.rs` програми `gameplay`. Ми надаємо собі доступ до типу програми для перевірки обліку та допоміжної функції для виклику CPI `create_metadata`.
...
Давайте створимо нову структуру для перевірки акаунтів з іменем `CreateCharacterSecure`. Цього разу зробимо `metadata_program` типом `Program`:
...
Наприкінці додамо інструкцію `create_character_secure`. Вона буде такою ж, як і раніше, але використовуватиме повні функціональні можливості Anchor CPIs, а не безпосередньо `invoke`:
...
Тепер, коли у нас є безпечний спосіб ініціалізації нового персонажа, давайте створимо новий тест. Цей тест просто має спробувати ініціалізувати атакуючого персонажа і очікувати, що виникне помилка.
...
Іноді вам може знадобитися більше гнучкості у CPI вашої програми. Ми, звісно, не перешкоджатимемо вам створювати програму, яка вам потрібна, але, будь ласка, вживайте всі можливі заходи безпеки, щоб уникнути вразливостей у своїй програмі.

Якщо ви хочете переглянути остаточний код рішення, ви можете знайти його в гілці `solution` в [тому ж репозиторії](https://github.com/Unboxed-Software/solana-arbitrary-cpi/tree/solution).

# Завдання

Так само, як і з іншими уроками цього блоку, найкраща можливість практикуватися полягає в аудиті власних або інших програм.

Приділіть час, щоб переглянути хоча б одну програму та переконайтеся, що перевірки програми виконуються для кожної програми, яка передається в інструкції, особливо тих, які викликаються через CPI.

Пам'ятайте, якщо ви знайдете помилку або вразливість в чужій програмі, будь ласка, повідомте про це! Якщо ж ви знайдете таке у своїй програмі, обов'язково виправте це негайно.
